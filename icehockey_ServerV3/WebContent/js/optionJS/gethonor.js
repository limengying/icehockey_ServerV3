$(function() {
	...
	var urlUserId=comm.getUrlParameter("userid");//解析url中的参数获取userid的值
		...
		// 请求后台保存数据
		$.post(GETHONORURL, data, function(result) {
			// 处理后台返回的结果
			var jsonReturn = JSON.parse(result);// 将JSON字符串转换为对象
			if (jsonReturn.result == "0") {
				var hresult = [];
				var trStr = '';//动态拼接table
				if(jsonReturn.honors.length!=0){				
				 for(var i=0; i<jsonReturn.honors.length; i++){
	                    hresult[i]=jsonReturn.honors[i];
	                    //alert("姓名："+hresult[i]);
	                    trStr += '<tr class="example">';//拼接处规范的表格形式
	                    trStr += '<td align="center">' + hresult[i].honorName + '</td>';//数据表的主键值
